...
/datum/dna/proc/generate_unique_features()
	...
	if(features["ethmark"])
		L[DNA_ETHEREAL_MARK_BLOCK] = construct_block(SSaccessories.ethereal_mark_list.Find(features["ethmark"]), length(SSaccessories.ethereal_mark_list))
...
/datum/dna/proc/update_uf_block(blocknumber)
	...
	switch(blocknumber)
		...
		if(DNA_ETHEREAL_MARK_BLOCK)
			set_uni_feature_block(blocknumber, construct_block(SSaccessories.ethereal_mark_list.Find(features["ethmark"]), length(SSaccessories.ethereal_mark_list)))
...
/mob/living/carbon/human/updateappearance(icon_update = TRUE, mutcolor_update = FALSE, mutations_overlay_update = FALSE)
	...
	if(dna.features["ethmark"])
		dna.features["ethmark"] = SSaccessories.ethereal_mark_list[deconstruct_block(get_uni_feature_block(features, DNA_ETHEREAL_MARK_BLOCK), length(SSaccessories.ethereal_mark_list))]
